...
1. Extraction of the human PMP34 sequence from the human proteome FASTA.  
2. Identification of orthologs via BLASTP searches against mammalian proteomes (Camacho et al., 2009).  
3. Consolidation of top scoring orthologs into a single FASTA file.  
4. Multiple sequence alignment using MUSCLE to reveal conserved regions and structural motifs (Edgar, 2004; Edgar, 2024).  
5. Computation of pairwise evolutionary distances based on the proportion of mismatched amino acids.  
6. Construction of a UPGMA phylogenetic tree using a custom Python implementation.  
...
Protein FASTA files were obtained from UniProtKB for the following species:
...
These proteomes were selected to represent a diverse set of mammalian lineages for comparative evolutionary analysis.

### 4.2 FASTA Parsing

Custom Python functions were used to read and write FASTA files. This approach ensures compatibility across proteomes and avoids reliance on external libraries. The parser extracts sequence headers and amino acid sequences while preserving formatting integrity.
...
This step identifies the closest homologous sequence in each proteome based on sequence similarity.

### 4.4 Ortholog Collection

The best BLAST hits were extracted from each proteome and consolidated into a single FASTA file named orthologs.fasta.  
This file serves as the input for multiple sequence alignment and downstream phylogenetic analysis.

### 4.5 Multiple Sequence Alignment

MUSCLE (v5) was executed via subprocess to generate a high quality multiple sequence alignment (Edgar, 2004; Edgar, 2024).  
Header normalisation was applied to remove MUSCLE generated suffixes and ensure consistent sequence identifiers across all stages of the pipeline.

### 4.6 Distance Matrix

Pairwise evolutionary distances were computed as the proportion of mismatched amino acids between aligned sequences.  
This metric reflects evolutionary divergence under the assumption of equal substitution rates and uniform evolutionary pressure across sites.

### 4.7 UPGMA Tree Construction

A custom UPGMA implementation was used to cluster sequences based on average linkage.  
The resulting hierarchical tree structure was exported in Newick format for compatibility with phylogenetic visualisation tools.
...
Each ortholog exhibited high sequence similarity to the human PMP34, confirming conserved function across taxa and validating the use of BLASTP for ortholog detection.

### 5.2 Multiple Sequence Alignment

The MUSCLE alignment revealed strong conservation of transmembrane helices and peroxisomal targeting motifs.  
These conserved regions are consistent with PMP34’s essential role in peroxisomal metabolite transport and support the hypothesis of functional conservation across mammals.

### 5.3 Phylogenetic Reconstruction

Figure 1. UPGMA phylogenetic tree of PMP34 orthologs across five mammalian species.  
The tree shows the evolutionary relationships among PMP34 orthologs from human, chimpanzee, gorilla, mouse, and dog.  
Branch lengths represent the fraction of amino acid mismatches.  
The primate sequences cluster tightly, while mouse and dog form more distant branches, reflecting earlier divergence.
